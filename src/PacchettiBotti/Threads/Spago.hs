...
updatePackageSets :: HasGitHub env => Message -> RIO env ()
updatePackageSets (NewRepoRelease address newTag) | address == PackageSets.packageSetsRepo = do
...
updatePackageSets _ = pure ()


-- | Whenever there's a new release of purescript, update the release on our various CI files
updatePurescriptVersion :: HasGitHub env => Message -> RIO env ()
updatePurescriptVersion (NewRepoRelease address newTag) | address == purescriptRepo = do
...
updatePurescriptVersion _ = pure ()


-- | Whenever there's a new release of the purescript-docs-search, update our version of it
updateDocsSearch :: HasGitHub env => Message -> RIO env ()
updateDocsSearch (NewRepoRelease address newTag) | address == docsSearchRepo = do
