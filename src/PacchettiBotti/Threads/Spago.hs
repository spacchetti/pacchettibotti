...
updatePackageSets :: GitHubEnv env am => am -> Message -> RIO env ()
updatePackageSets token (NewRepoRelease address newTag) | address == PackageSets.packageSetsRepo = do
  let prTitle = "Update to package-sets@" <> newTag
  let prBranchName = "spacchettibotti-" <> newTag
  Run.runAndOpenPR token GitHub.SimplePR{ prBody = "", prAddress = spagoRepo, ..} (const $ pure ())
    [ "cd templates"
    , "spago upgrade-set"
    , "git add packages.dhall"
    ]
updatePackageSets _ _ = pure ()


-- | Whenever there's a new release of purescript, update the release on our various CI files
updatePurescriptVersion :: GitHubEnv env am => am -> Message -> RIO env ()
updatePurescriptVersion token (NewRepoRelease address newTag) | address == purescriptRepo = do
...
updatePurescriptVersion _ _ = pure ()


-- | Whenever there's a new release of the purescript-docs-search, update our version of it
updateDocsSearch :: GitHubEnv env am => am -> Message -> RIO env ()
updateDocsSearch token (NewRepoRelease address newTag) | address == docsSearchRepo = do
