...
checkLatestRelease :: HasEnv env => PackageName -> GitHub.Address -> RIO env ()
checkLatestRelease packageName address = GitHub.getLatestRelease address >>= \case
  Left err -> logWarn $ "Could not check the latest release for " <> displayShow address <> ". Error: " <> displayShow err
  Right GitHub.Release {..} -> do
    DB.transact (DB.getLatestRelease packageName) >>= \case
      -- We don't do anything if we have a release saved and it's the current one
      Just latestRelease | Tag releaseTagName == latestRelease -> pure ()
      _ -> do
